...
* map - a map with keys denoting operating system and value either in _plain_ or _block_ form, for instance +[linux:"foo", windows: {"foo"}]+. Following OS values are supported:
** +linux+
** +windows+
** +mac+
** +solaris+
+ 
NOTE: If you are using map, you need to use parenthesis around parameter
...
profiles::
    Profiles consist of installations to be installed and tests to be executed. You can use -P_profileName_ to trigger
...
installationName {
    ...
    extractMapper { any ant mapper }
    tool {
        name = 'toolName' // must not contain - or space
        command value
    }
    postActions {
    }
